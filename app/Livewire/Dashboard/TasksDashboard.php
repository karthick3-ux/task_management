...
protected $messages = [
    ...
    public function render()
    {
      ...
      ->addSelect([
    'latest_date' => TaskAssignment::selectRaw(
        'GREATEST(COALESCE(MAX(deadline), "0000-00-00"), COALESCE(MAX(expected_date), "0000-00-00"))'
        ...
        $tasks = $tasksQuery->orderBy($this->sortBy, $this->sortDirection)
                           ->paginate($this->displayMode === 'table-full' ? 15 : 10);

        $this->updateStatistics();
        $this->loadCalendarEvents();

        return view('livewire.dashboard.tasks-dashboard', [
            'tasks' => $tasks,
        ])->layout('layouts.app');
    }
   ...
   private function loadCalendarEvents()
{
    ...
    $assignments = TaskAssignment::with(['task.project', 'user'])
        ...
        ->whereHas('task',function($query){
            $query->whereNotIn('task_status',['Completed']);
        })
        ->where('is_admin',0)
        ->get();
    ...
    foreach ($assignments as $assignment) {
        ...
        if(!empty($assignment->expected_date)){
        if($assignment->deadline >= $assignment->expected_date)
             $startDateStr = $assignment->deadline->format('Y-m-d');
        else 
            $startDateStr = $assignment->expected_date->format('Y-m-d');
    ...
    foreach ($tasksByDate as $date => $tasks) {
        foreach ($tasks as $taskId => $taskData) {
            ...
            foreach ($taskData['assignments'] as $assignment) {
                if (($assignment->deadline < Carbon::today() || $assignment->expected_date > $assignment->deadline) && 
    ...
    public function moveAssignmentUp2($index)
    {
        ...
        if ($index > 0) {
         ...
         $this->PropagateExpectedDateAssignment();
    }
   ...
   public function saveAssignments()
                    ...
                    if (isset($assignmentData['id']) && $assignmentData['id']) {
                        ...
                        if ($assignment) {
                            ...
                            $canEdit = auth()->user()->isSuperAdmin() || 
                                      $assignment->user_id == auth()->id() || auth()->user()->can('assign_task_users');
                            ...
                            if ($canEdit) {
                                ...
                                $this->logIndividualAssignmentChanges($assignment, $originalValues, $assignmentData,$index);
    ...
    private function logIndividualAssignmentChanges($assignment, $originalValues, $newData,$index)
    {
        ...
        if (isset($originalValues['status']) && isset($newData['status']) && 
            ...
            if($newData['status']=='Completed'){
                ...
                if(isset($this->assignmentData[$nex_index]) && $this->assignmentData[$nex_index]['id']){
                    $a=TaskAssignment::find($this->assignmentData[$nex_index]['id']);
                      $historyService->logNextAssignmentStart(
              $a
            );
                }
    ...
    public function handleStatusChange($index, $newStatus)
    {
        ...
        $canEdit = auth()->user()->isSuperAdmin() || 
                   (isset($assignment['user_id']) && $assignment['user_id'] == auth()->id()) || auth()->user()->can('assign_task_users');
...
public function addTaskAssignment($afterIndex = null)
{
     ...
     if (!empty($this->taskAssignments[0]['start_date'])) {
                    $start_date = $this->taskAssignments[0]['start_date'];
                }
 ...
 */
public function PropagateExpectedDateRow()
{

      // Propagate expected date to all subsequent assignments
   
    for ($i = 1; $i < count($this->taskAssignments); $i++) {
        if(isset($this->taskAssignments[$i])){
            if(!empty($this->taskAssignments[$i-1]['no_of_days']) && !empty($this->taskAssignments[$i-1]['expected_date'])){
                   $expectedDate = Carbon::parse($this->taskAssignments[$i-1]['expected_date']);
                    $prevDays = (int) $this->taskAssignments[$i-1]['no_of_days'];
                    $expectedDate =  $expectedDate->addDays($prevDays);
        $this->taskAssignments[$i]['expected_date'] = $expectedDate->format('Y-m-d');
            }
        }
    }
...
public function PropagateExpectedDateAssignment()
{

      // Propagate expected date to all subsequent assignments
   
    for ($i = 1; $i < count($this->assignmentData); $i++) {
        if(isset($this->assignmentData[$i])){
            if(!empty($this->assignmentData[$i-1]['no_of_days']) && !empty($this->assignmentData[$i-1]['expected_date'])){
                   $expectedDate = Carbon::parse($this->assignmentData[$i-1]['expected_date']);
                    $prevDays = (int) $this->assignmentData[$i-1]['no_of_days'];
                    $expectedDate =  $expectedDate->addDays($prevDays);
        $this->assignmentData[$i]['expected_date'] = $expectedDate->format('Y-m-d');
            }
        }
    }
...
public function addAssignmentRow()
{
    ...
    $nextSequence = count($this->assignmentData) + 1;
     ...
     if (!empty($this->assignmentData[0]['start_date'])) {
                    $start_date = $this->assignmentData[0]['start_date'];
                }
...
private function validateAssignments()
{
    ...
    foreach ($this->assignmentData as $index => $assignment) {
        if (!empty($assignment['start_date']) && !empty($assignment['deadline'])) {
            ...
            if ($startDate->gt($deadline)) {
                  session()->flash('error', "Start date cannot be later than deadline for assignment " . ($index + 1));
        ...
        if (!empty($assignment['expected_date']) && !empty($assignment['deadline'])) {
            ...
            if ($expectedDate->gt($deadline)) {
                                  session()->flash('error', "Expected date cannot be later than deadline for assignment " . ($index + 1));
